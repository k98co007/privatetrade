...
| LLD 완료 | 실무 개발자, LLD 테스트 문서, LLD 테스트 환경 담당자 | 병렬 티켓 3개 발행 |
| 개발 완료 | CI/CD 담당자 | 빌드/배포 티켓 |
| 배포 완료 | HLD/LLD 테스트 운영 담당자 | 테스트 수행 티켓 |
...
| LLD | 인터페이스·시퀀스·제약 완성, 수도코드 실행 가능, 개발자 검토 승인 |
| 테스트 케이스 | 커버리지 ≥ 80%, 명확성·재현성 확보 |
| 코드 | 빌드 성공, 충분한 로그, 코드 리뷰 승인 |

#### 7. 버그 트리아주 및 피드백 루프
- **버그 분류**: 코드/설계(LLD)/아키텍처(HLD)/요구사항(SRS/유저스토리)
